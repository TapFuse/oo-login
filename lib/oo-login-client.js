...
Template.loginControl.events({
  ...
  'click .js-logInAsGuest'(e, t) {
    if (confirm("Warning, if you login anonymously, you won't be able to access the same account on another device, unless you connect your social media account on your profile settings page later.")) {
      t.loginInProcess.set(true);
      Meteor.call('appCreateUserAndTicket', function(err, result) {
        if (result) {
          if (result && result.username && result.pwd) {
            console.log('%c result.username   ', 'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;', result.username);
            Meteor.loginWithPassword(result.username, result.pwd, function(err1) {
              if (err1) {
                t.loginInProcess.set(false);
                console.log(err1);
                alert(err1.reason);
