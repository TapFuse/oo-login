LOGIN_CODE_TIMEOUT_MINUTES = 1440;
...
function loginUserWithNewPassword(callbackObj, emailPassed) {
  const email = emailPassed.toLowerCase();
  const user = Meteor.users.findOne({'emails.address': email});
  const password = Random.hexString(30);
  let userId;
  if ( !user ) {
    console.log('user was NOT FOUND, loginUserWithNewPassword');
...
Meteor.methods({
  ...
  enter_security_code: function(userEmail, code) {
    ...
    const password = loginUserWithNewPassword(this, email);
